...
def load_dataset_stats(config):
    ...
    if config.data.dataset == 'CIFAR10':
        filename = config.eval.fid_dir + '/cifar10_stats.npz'
    elif config.data.dataset == 'CELEBA':
        filename = config.eval.fid_dir + 'celeba_stats.npz'
    elif config.data.dataset == 'CelebAHQ':
        filename = config.eval.fid_dir + 'CelebAHQ.npz'
...
def run_inception_distributed(input_tensor,
    ...
    for i, tensor in enumerate(input_tensors):
        with tf.device(device_format.format(i)):
            ...
            res = run_inception_jit(tensor_on_device, inception_model, num_batches=num_batches,
    ...
    with tf.device('/CPU'):
        return {'pool_3': tf.concat(pool3, axis=0),
