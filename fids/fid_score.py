...
from fids.inception import InceptionV3
...
def evaluate_fid(config, samples, logger):    
    dims = 2048
    device = config.device
    num_gpus = 1
    num_sample_per_gpu = int(np.ceil(config.eval.num_samples / num_gpus))

    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    model = InceptionV3([block_idx], model_dir=config.eval.fid_dir).to(device)
    m, s = calculate_activation_statistics(samples, model, config.eval.batch_size, dims, device, max_samples=num_sample_per_gpu)
    ...
    path = os.path.join(config.eval.fid_dir, config.data.dataset + '.npz')
